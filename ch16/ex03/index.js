...
function encrypt64(text, key) {
  ...
  const encryptedBase64 = cipher.final("base64");
...
async function writeEncrypt64(data) {
  ...
  await fs.writeFile("./ch16/ex03/encrypt.json", JSON.stringify(data));
}

async function readKey() {
  // ここを埋める（return Promise<鍵>）
  const data = await fs.readFile("./ch16/ex03/key.json", "utf8");
...
async function readEncrypt64() {
  ...
  const data = await fs.readFile("./ch16/ex03/encrypt.json", "utf8");
