...
// UserオブジェクトUser: { id: number, name: string }
class User {
  id: number;
  name: string;
...
// TaskオブジェクトTask: { title: string, completed: boolean, user: User }
class Task {
  title: string;
  completed: boolean;
  user: User;
...
// Priorityは"low"|"middle"|"high"のいずれかの値をとる
type Priority = "low" | "middle" | "high";

// PriorityTaskはTaskかつ{ priority: Priority }を持つ型
